...
var validate = new (function () {
  ...
  this.domain = function (domain) {
    return typeof domain === "string" && domain === "is-a.tech";
  };
  ...
  this.a = function (a) {
    return (
      typeof a === "string" &&
      /^((25[0-5]|(2[0-4]|1\d|[1-9]|)\d)\.?\b){4}$/.test(a)
    );
  };
  ...
  this.aaaa = function (aaaa) {
    return (
      typeof aaaa === "string" &&
      /^([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}$/.test(aaaa)
    );
  };
...
function addSubDomain(data) {
  ...
  if (!validate.description(data.description)) {
    throw new Error(
      'Invalid subdomain description: "' + data.description + '"'
    );
  }
  ...
  if (typeof data.owner === "object") {
    ...
    if (
      data.owner.email !== undefined &&
      typeof data.owner.email !== "string"
    ) {
  ...
  if (typeof data.record !== "object") {
    ...
    if (Array.isArray(data.record.AAAA)) {
      if (!data.record.AAAA.every(validate.aaaa)) {
        throw new Error(
          "AAAA records must be an array of valid IPv6 addresses"
        );
      }
    }
    ...
    if (typeof data.record.CNAME === "string") {
      if (!validate.cname(data.record.CNAME)) {
        ...
        data.record.CNAME =
          data.record.CNAME.toLowerCase().replace(/\.+$/, "") + "."; // normalize and add trailing dot
      }
    }
    ...
    if (Array.isArray(data.record.NS)) {
      if (
        ...
        throw new Error(
          "NS records cannot be used with A, AAAA or CNAME records"
        );
      }
  ...
  if (Array.isArray(data.nested)) {
    for (var i = 0; i < data.nested.length; i++) {
      if (!validate.subDomain(data.nested[i].subdomain)) {
        throw new Error(
          'Invalid nested subdomain name: "' + data.nested[i].subdomain + '"'
        );
      }
      ...
      if (typeof data.nested[i].record.CNAME === "string") {
        if (!validate.cname(data.nested[i].record.CNAME)) {
          throw new Error(
            'Invalid CNAME record: "' + data.nested[i].record.CNAME + '"'
          );
        } else {
          data.nested[i].record.CNAME =
            data.nested[i].record.CNAME.toLowerCase().replace(/\.+$/, "") + "."; // normalize and add trailing dot
        }
      }
      ...
      if (Array.isArray(data.nested[i].record.NS)) {
        if (!data.nested[i].record.NS.every(validate.ns)) {
          ...
          data.nested[i].record.NS = data.nested[i].record.NS.map(function (
            ns
          ) {
            return ns.toLowerCase().replace(/\.+$/, "") + "."; // normalize and add trailing dot
          });
...
subDomains.forEach(function (subDomain) {
  if (commit[subDomain.domain] === undefined) {
    // initialize domain
  ...
  if (subDomain.record.CNAME) {
    commit[subDomain.domain].push(
      CNAME(subDomain.subdomain, subDomain.record.CNAME, proxy)
    );
  }
    ...
    subDomain.nested.forEach(function (nested) {
      ...
      if (nested.record.AAAA) {
        nested.record.AAAA.forEach(function (aaaa) {
          commit[subDomain.domain].push(
            AAAA(nestedSubdomain, aaaa, nestedProxy)
          );
        });
      }
      ...
      if (nested.record.CNAME) {
        commit[subDomain.domain].push(
          CNAME(nestedSubdomain, nested.record.CNAME, nestedProxy)
        );
      }
